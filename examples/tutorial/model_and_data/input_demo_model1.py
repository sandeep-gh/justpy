...
async def input_demo_model1(request):
    ...
    myinput = jp.Input(a=wp, classes=input_classes, placeholder='Please type here', model=[wp, 'text'])
    def on_change(dbref, msg):
        print (msg.value)
        
    myinput.on("change", on_change)
    jp.Div(model=[wp, 'text'], classes='m-2 p-2 h-32 text-xl border-2 overflow-auto', a=wp)
    return wp

# initialize the demo
from  examples.basedemo import Demo
Demo (input_demo_model1)
